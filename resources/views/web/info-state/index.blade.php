...
@section('title', 'Info States - Admin')
        ...
        <div class="col-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Info States</h3>
                </div>
                <div class="panel-body">
                    ...
                    <form method="GET" class="form-inline mb-3">
                        ...
                        <a href="{{ route('web.info-state.index') }}" class="btn btn-default ml-2">Clear</a>
                    </form>

                    <!-- Export Button -->
                    <div class="mb-3">
                        <a href="{{ route('web.info-state.export', request()->query()) }}" class="btn btn-success">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse($infoStates as $infoState)
                                    <tr>
                                        <td>{{ $infoState->id }}</td>
                                        <td>
                                            {{ $infoState->key }}
                                            @if(isset($subscriptions[$infoState->key]))
                                                ({{ $subscriptions[$infoState->key] }})
                                            @endif
                                        </td>
                                        <td>{{ $infoState->created_at->format('Y-m-d') }}</td>
                                        <td>{{ $infoState->updated_at->format('Y-m-d') }}</td>
                                        <td>
                                            <div class="btn-group">
                                                <a href="{{ route('web.info-state.show', $infoState) }}" class="btn btn-xs btn-info">
                                                    <i class="fas fa-eye fa-fw"></i>
                                                </a>
                                                <form method="POST" action="{{ route('web.info-state.destroy', $infoState) }}" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center">No info states found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="d-flex justify-content-center">
                        {{ $infoStates->appends(request()->query())->links() }}
                    </div>
                </div>
            </div>
        </div>
